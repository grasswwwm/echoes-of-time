window.addEventListener("DOMContentLoaded", () => {
  ...
  infoPopup.addEventListener("click", (e) => {
    if (e.target === infoPopup) infoPopup.classList.remove("show");
  });


  // 🖱️ Smooth fading mouse trail
  ...
  // 🧱 Matter.js setup for falling lyrics
  ...
  function spawnLyric(lyric) {
    ...
    const numWords = Math.ceil(Math.random() * 3) + 1;
    const selected = [];

    while (selected.length < numWords && words.length > 0) {
    ...
    selected.forEach((word) => {
      const wordWidth = word.length * (fontSize * 0.4) + 40;
      let startX, overlap;
      do {
        overlap = false;
      ...
      } while (overlap && usedPositions.length < 50);

      usedPositions.push([startX, startX + wordWidth]);

      const startY = 100 + Math.random() * (screenHeight * 0.3);
      let currentX = startX;

      for (let i = 0; i < word.length; i++) {
        const ch = word[i];
        const body = Bodies.circle(currentX, startY, 10, {
          restitution: 0.6,
          friction: 0.1,
          density: 0.0005,
  ...
  (function renderText() {
    ...
    for (const body of world.bodies) {
      if (body.labelText) {
        ...
        ctx.fillStyle = color === "black" ? `rgba(0,0,0,${alpha})` : `rgba(179,179,179,${alpha})`;
